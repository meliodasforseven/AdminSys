...
app.post(`/verify-request`, async (request, response) => {
    ...
    if(successStatus == "true") {
        if("moderator" in request.headers) {
            ...
            embed.addField("معلومات الباند", `**Moderator**: ${request.headers.moderator}\n**السبب**: ${request.headers.reason}`);
            channel.send(embed);
        } else {
            channel.send(`<@${commandRequest.authorID}>`);
            channel.send(client.embedMaker(client.users.cache.get(commandRequest.authorID), "تم الامر بنجاح", message));
        }
    } else {
        channel.send(`<@${commandRequest.authorID}>`);
        channel.send(client.embedMaker(client.users.cache.get(commandRequest.authorID), "فشل الامر", message));
    }
